...
std::string Address::HostStr(std::size_t length)
{
    std::string host {};
    host.reserve(length);
    assertm(enet_address_get_host(&enetAddress, &host[0], length) == 0, "Cannot get Host");// remindme: add unit test
    return host;
}
...
std::string Address::Ip(std::size_t length)
{
    std::string ip {};
    ip.reserve(length);
    assertm(enet_address_get_host_ip(&enetAddress, &ip[0], ip.size()), "Cannot get ipaddress"); // remindme: add unit test
    return ip;
}
